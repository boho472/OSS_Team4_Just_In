...
def compute_Z_from_mask(depth_map, mask_full):
    ...
    # 2단계: 실패하면 전체 마스크 사용 (NEW!)
    if len(xs) < 50:  # 픽셀이 너무 적으면
        print(f"⚠️ Middle 40% failed, using full mask")
        ys, xs = np.where(mask_full > 0)
...
def convert_to_3D(txt_path,boxes,masks,depth_map,scores,save_json_path,file_name):
    out = open(txt_path, "w")
    used_Z = set()

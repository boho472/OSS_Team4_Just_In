...
class KittiTrackingDataset:
    def __init__(self,root_path,saved_path,seq_id,ob_path = None,load_image=False,load_points=False,type=["Car"]):
        ...
        self.all_ids = [f for f in os.listdir(self.image_path) if f.endswith('.jpg') or f.endswith('.png')]
        self.all_ids.sort()
    ...
    def __getitem__(self, item):
        ...
        if self.ob_path is not None:
            ob_path = os.path.join(self.ob_path, name[:-4] + '.txt')

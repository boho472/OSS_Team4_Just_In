...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_to_drm(
        ...
        # Adding to DRM is implemented via conditioning frames in SAM2
        storage_key = "cond_frame_outputs"

        inference_state["adds_in_drm_per_obj"][obj_idx].append(frame_idx)
        # Add the output to the output dict (to be used as future memory)
        # Get stored output from this frame (from propagate_in_video)
        obj_temp_output_dict[storage_key][frame_idx] = self.curr_out
    ...
    def _consolidate_temp_output_across_obj(
        ...
        for obj_idx in range(batch_size):
            ...
            consolidated_out["n_pixels_pos"][obj_idx: obj_idx +
                                             1] = out["n_pixels_pos"]
            consolidated_out["iou"][obj_idx: obj_idx +
                                    1] = torch.tensor(out["iou"])
    ...
    def propagate_in_video_preflight(self, inference_state):
        ...
        for adds_in_drm_per_frame in inference_state["adds_in_drm_per_obj"].values():
            # Frames that were added to the DRM part of memory
            input_frames_inds.update(adds_in_drm_per_frame)
    ...
    def propagate_in_video(
        ...
        for frame_idx in processing_order:
            ...
            # Save the current output if it will be used in the DRM memory
            self.curr_out = current_out
            ...
            if return_all_masks and "all_pred_masks" in output_dict[storage_key][frame_idx]:
                ...
                all_pred_masks = torch.tensor(
                    output_dict[storage_key][frame_idx]["all_pred_masks"][0])
                ...
                for pred_mask_idx in range(len(all_pred_masks)):
                    _, video_res_masks = self._get_orig_video_res_output(
                        inference_state, all_pred_masks[pred_mask_idx].unsqueeze(
                            0).unsqueeze(0)
                    )
    ...
    def _run_single_frame_inference(
        ...
        current_out["n_pixels_pos"] = (
            pred_masks_gpu[0, 0] > 0.0).detach().cpu().numpy().astype(np.uint8).sum()
        current_out["iou"] = 1.0
